...
export default function RecentActivity() {
  ...
  return (
    <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl  border border-gray-200/50 dark:border-gray-700/50 p-6 md:p-8 transition-all duration-500 overflow-hidden">
      ...
      <div className="relative z-10 flex items-center justify-between mb-8">
        ...
        <button className="group flex items-center space-x-2 space-x-reverse text-sm font-semibold text-primary-600 dark:text-primary-400  px-4 py-2 rounded-xl ">
          <span>{t('viewAll')}</span>
        ...
        {activities.map((activity, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Activity Card */}
              <div className={`relative  bg-gradient-to-br ${activity.bgGradient} dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 border ${activity.borderColor} shadow-md `}>
                {/* Shine Effect */}
                
                <div className="relative flex items-start space-x-4 space-x-reverse">
                  {/* Icon Container */}
                  <div className={`relative mx-3 flex-shrink-0 w-14 h-14 rounded-2xl bg-gradient-to-br ${activity.gradient} shadow-lg flex items-center justify-center `}>
                ...
                {/* Hover Border Effect */}
                <div className={`absolute inset-0 rounded-2xl border-2 ${activity.borderColor} opacity-0 transition-opacity duration-300 pointer-events-none`}></div>
              </div>
            </div>
          )
        })}
