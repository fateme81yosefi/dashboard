...
export default function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed lg:static right-0 top-0 z-50 h-full w-64 bg-white dark:bg-gray-800 shadow-lg transform transition-transform duration-300 ease-in-out',
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-t border-gray-200 dark:border-gray-700">
            <div className="bg-gradient-to-r from-primary-500 to-primary-600 rounded-lg p-4 text-white">
              <h3 className="font-bold mb-1">{t('newProject')}</h3>
              <p className="text-sm text-primary-100 mb-3">
                {t('startProject')}
              </p>
              <button className="w-full bg-white text-primary-600 py-2 rounded-lg font-medium hover:bg-primary-50 transition-colors">
                {t('startNow')}
              </button>
            </div>
          </div>
